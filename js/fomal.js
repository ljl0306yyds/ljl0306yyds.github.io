...
function createtime2() {
  ...
  setTimeout(
    console.warn.bind(
      console,
      "%c ⚡ Powered by Fomalhaut🥝 %c 你正在访问iTddey🥝の小家",
...
function share_() {
  ...
  try {
    ...
    var subTitle = title.endsWith("| iTddey🥝") ? title.substring(0, title.length - 14) : title;
    navigator.clipboard.writeText('iTddey🥝的站内分享\n标题：' + subTitle + '\n链接：' + url + '\n欢迎来访！🍭🍭🍭');
    new Vue({
      data: function () {
        this.$notify({
          title: "成功复制分享信息🎉",
...
document.addEventListener('visibilitychange', function () {
  if (document.hidden) {
    //离开当前页面时标签显示内容
    document.title = '👀网页遭病毒入侵啦~';
    clearTimeout(titleTime);
  } else {
    //返回当前页面时标签显示内容
    document.title = '🐖呜呼呼，又好啦～';
    //两秒后变回正常标题
